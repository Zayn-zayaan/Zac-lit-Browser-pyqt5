...
class App(QFrame):
    ...
    def SwitchTab(self, i):
        ...
        if self.tabbar.tabData(i) is None:
            td = self.findChild(QWidget, self.tabs[i].objectName())
            self.container.layout.setCurrentWidget(td)
            new_url = td.content.url().toString()
            self.addressbar.setText(new_url)
